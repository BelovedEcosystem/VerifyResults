(* E0_LLDUC_equations.v
   Self-contained Coq 8.12+ / 8.18. Stdlib only.
   Proves the 504-web as Q-arithmetic. Does NOT prove
   LB(H_LLDUC, a_star), P30.E0, or that 504 is spectral.
*)
...
(* Gen, weight index: eis(w) = -2w / B_w *)
Definition eis_Gen (w : nat) : Q :=
  -(inject_Z (2 * Z.of_nat w)) / bern w.

(* ZetaBridge, half-weight: eis(k) = -4k / B_{2k} *)
...
Theorem a_star_le_head : a_star <= head.
Proof. vm_compute. reflexivity. Qed.

Theorem a_star_inherits_downward :
  forall P : Q -> Prop,
    downward_closed P -> P head -> P a_star.
Proof.
  intros P Hdc Ph.
  apply (Hdc a_star head).
  - apply a_star_le_head.
  - exact Ph.
Qed.
